...
public class WorkingMessage {
	...
	/**
	 * Returns true if the message has any text. A message with just whitespace
	...
	public int setAttachment(int type, Uri dataUri, boolean append) {
		...
		if (!MmsConfig.getMultipartSmsEnabled()) {
			if (!append && mAttachmentType == TEXT && type == TEXT) {
				...
				/*
				 * SmsMessage.calculateLength returns an int[4] with: int[0]
				 * being the number of SMS's required, int[1] the number of code
				 * units used, int[2] is the number of code units remaining
				 * until the next message. int[3] is the encoding type that
				 * should be used for the message.
				 */
	 ...
	 * only for single-slide ("attachment mode") messages.
	 */
	private void changeMedia(int type, Uri uri) throws MmsException {
		...
		if (type == IMAGE) {
			media = new ImageModel(mActivity, uri, mSlideshow.getLayout()
					.getImageRegion());
		} else if (type == VIDEO) {
			media = new VideoModel(mActivity, uri, mSlideshow.getLayout()
					.getImageRegion());
		} else if (type == AUDIO) {
	...
	private CharSequence removeDiacritics(CharSequence sequence) {
		...
		for (int i = 0; i < msgLength; i++) {
			...
			msgReplacement = msgReplacement.concat(String.valueOf(c));
		}

		return (CharSequence) msgReplacement;
	}
	...
	public void send(final String recipientsInUI) {
        ...
        int[] params = SmsMessage.calculateLength(mText, true);
        int msgCount = params[0];
        int remainingInCurrentMessage = params[2];
        
        if (!requiresMms()) {
        	// only if not mms
	        if (msgCount == 1 && remainingInCurrentMessage > 90) {
	        	Log.i(TAG, "sending <70 sms");
	        	// message is shorter than 70 signs - send diacritics
	        } else if (msgCount == 1 && remainingInCurrentMessage <= 90) {
	        	Log.i(TAG, "sending 70-160 sms");
	        	// there is one message with more than 70 signs - remove diacritics
	        	mText = removeDiacritics(mText);
	        } else if (msgCount > 1) {
	        	Log.i(TAG, "sending 160- sms");
	        	// sending more than 1 sms
	        	mText = removeDiacritics(mText);
	        }
        }
	...
	private static Uri readDraftMmsMessage(Context context, Conversation conv,
			StringBuilder sb) {
		if (Log.isLoggable(LogTag.APP, Log.VERBOSE)) {
		...
		try {
			if (cursor.moveToFirst()) {
				...
				String subject = MessageUtils.extractEncStrFromCursor(cursor,
						MMS_SUBJECT_INDEX, MMS_SUBJECT_CS_INDEX);
				if (subject != null) {
					sb.append(subject);
				}
				if (Log.isLoggable(LogTag.APP, Log.VERBOSE)) {
	...
	private static Uri createDraftMmsMessage(PduPersister persister,
			SendReq sendReq, SlideshowModel slideshow) {
		try {
